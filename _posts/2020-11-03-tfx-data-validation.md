...
TFDV can be used for generating schemas and statistics about the distribution of every feature in the dataset. Such information are useful comparing multiple datasets (e.g. trainig vs inference datasets) and reporting:
...
* Update the schmea if needed

Then when a new data comes in, the workflow becomes:
* Generate statistics and schema for the new data.
* Visualize side by side the statistics to the training data statistics.
* Validate the statistics against the one from training to detect anomalie.
...
You can notice that TFDV generates different types of statitics based on the type of features.
...
Once statistics are generated, the next step is to generate a schema for our dataset. This schema will map each feature in the dataset to a type (float, bytes, etc.). Also define feature bounderies (min, max, distribution of values and missings, etc.).
...
### Updating schema
TFDV let you update the schema according to your domain knowledge of the data if you are not satified by the auto-generated schema.
...
### Comparing Datasets
Suppose we have two datasets one for training the over for evaluation. TFDV let's you determine how representative is the evaluation dataset of to the training one. More precisely, it helps you answer questions like:
* Does the evaluation data have similar schema as training dataset?
...
In this example, we can easily see that the distribution of the two datasets is very different. In most cases, they same to follow a normal distribution but clearly with two differnt mean and standard diviation. We can also see that in the second dataset, the  numerical feature **X4** has lot more missing values than it does in the first dataset. Also, the categotical feature **X3** seem to have an additional label in the sesond dataset which is not present in the first dataset.
...
### Comparing slices
In addition to comparins entire dataets, TFDV can also be used to compare slices of the same dataset on a particular feature. This is very useful when inspect the data for bias when missing values are not uniformly spread over the different labels.
...
## Using TFDV with TFX
In the previous sections, we have seen how to use TFDV for exploring and validation datsets as a standalone tool. This is very handy for manual inspection of the data, but this can also be automated in TFX using the following components:

### Generating statistics with StatisticsGen
For generating statistics, TFX provides the pipeline component `StatisticsGen`. This components accepts as input the output from the pipeline component `ExampleGen` components as input. It can be used as follows:
...
Note: if a schema already exsits in the metadata store, the SchemaGen will not genrate a new one and in case it has to be updated (e.g. due to presence of a new feature) you may have to update it manually as explained earlier.
...
Note: in case of any anomalies detected by the `ExampleValidator` component (i.e. mismatches in statistics or schema) it will set the status of the pipeline in the medata store to failed, which will eventually stop it. Otherwise, the pipeline will proceed, for instance to the data preprocessing step.